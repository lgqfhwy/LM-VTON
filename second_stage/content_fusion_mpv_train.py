...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        ############## Forward Pass ######################
        losses, img_hole_hand, arm_label_map, warped_cloth, skin_color, fake_image, real_image, label_parse = model.forward(Variable(data['label_parse']), Variable(data['warped_cloth']), Variable(data['arm_label_map']),
             Variable(data['fake_label_cloth_mask']), Variable(data['remove_arm_parse']), Variable(data['label_cloth_mask']), 
            Variable(data['image']), Variable(data['pose_map']), Variable(data['all_label_person_shape']), Variable(data['cloth']), infer=save_fake)
        ...
        if save_fake:
            ...
            visuals = OrderedDict([('origin_arm_label_map', util.tensor2label(data['arm_label_map'][0], 20)),
                                    ('label_parse', util.tensor2label(data['label_parse'][0], 20)),
