...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_attention_Refine(self.content_input, 3, block_version = opt.block_version,gpu_ids=self.gpu_ids)
        ...
        if self.isTrain:
            ...
            # self.netD = networks.define_attention_D(self.content_input + 3, opt.ndf, opt.n_layers_D, opt.block_version, opt.norm, use_sigmoid, 
            #                     opt.num_D, not opt.no_ganFeat_loss, gpu_ids=self.gpu_ids)
    ...
    def forward(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, infer=False):
    ...
    def inference(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape):
        # Encode Inputs 
        self.netG.eval()       
        label_parse = label_parse.cuda()
        warped_cloth = warped_cloth.cuda()
