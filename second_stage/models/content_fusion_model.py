...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.no_semantic = opt.no_semantic
        self.no_skin_color = opt.no_skin_color
        self.no_skin_color_and_semantic = opt.no_skin_color_and_semantic
        ...
        if self.no_semantic:
            self.content_input = 9
        elif self.no_skin_color:
            self.content_input = 26
        elif self.no_skin_color_and_semantic:
            self.content_input = 6
    ...
    def forward(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth, infer=False):
        ...
        # bigger_arm1_occ = morpho(arm1_occ, 20)
        # bigger_arm2_occ = morpho(arm2_occ, 20)
        ...
        img_hole_hand = real_image * (1 - label_cloth_mask)
        ...
        elif self.no_semantic:
            G_in = torch.cat([img_hole_hand, warped_cloth, skin_color], 1)
        elif self.no_skin_color:
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth], 1)
        elif self.no_skin_color_and_semantic:
            G_in = torch.cat([img_hole_hand, warped_cloth], 1)
        else:
        ...
        return [ self.loss_filter( loss_G_GAN, loss_G_other, loss_D_real, loss_D_fake ), None if not infer else img_hole_hand,
                                                                                      ...
                                                                                      None if not infer else label_parse]
    def inference(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth):
        ...
        only_remove_cloth = real_image * (1 - label_cloth_mask)
        # print("img_hole_hand = ", img_hole_hand)
        # print("dis_label = ", dis_label)
        # print("warped_cloth = ", warped_cloth)
        # print("skin_color = ", skin_color)
        ...
        # else:
        #     fake_image = self.netG.forward(input_concat)
        return img_hole_hand, arm_label_map, warped_cloth, skin_color, fake_image, real_image, only_remove_cloth
