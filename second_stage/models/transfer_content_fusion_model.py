...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.add_origin_cloth = opt.add_origin_cloth

        if self.add_noise:
            self.content_input += 1
        if self.add_origin_cloth:
            self.content_input += 3        
    ...
    def forward(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth, infer=False):
        ...
        if self.add_origin_cloth:
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color, cloth], 1)
        else:
        ...
        pred_fake_pool_D = self.discriminate_D(G_in, fake_image, use_pool=True)
        ...
        pred_real_D = self.discriminate_D(G_in, real_image)
        loss_D_real = self.criterionGAN(pred_real_D, True)

        # # GAN loss (Fake Passability Loss)        
        pred_fake_D = self.netD.forward(torch.cat((G_in, fake_image), dim=1))        
        loss_G_GAN = self.criterionGAN(pred_fake_D, True)

        # print("loss_l1 = ", loss_l1)
        # print("loss_vgg = ", loss_vgg)
        loss_G_other = loss_l1 + loss_vgg * self.vgg_weight
    ...
    def inference(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth):
        # Encode Inputs 
        self.netG.eval()       
        label_parse = label_parse.cuda()
        warped_cloth = warped_cloth.cuda()
        ...
        # print("warped_cloth = ", warped_cloth)
        # print("skin_color = ", skin_color)
        #assert torch.all(origin_fake_label_cloth_mask == dis_label)
        #G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color], 1)
        if self.add_origin_cloth:
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color, cloth], 1)
        else:
    ...
    def transfer_inference(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth):
        # Encode Inputs
        self.netG.eval()
        transfer_g = copy.deepcopy(self.netG)    
        label_parse = label_parse.cuda()
        warped_cloth = warped_cloth.cuda()
        arm_label_map = arm_label_map.cuda()
        fake_label_cloth_mask = fake_label_cloth_mask.cuda()
        ...
        if self.add_origin_cloth:
            ...
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color], 1)
        origin_output, output = self.train_transfer_input(transfer_g, self.netD, G_in, cloth, label_cloth_mask)
        # with torch.no_grad():
        #     fake_image = self.netG.refine(G_in)
        # fake_image = self.tanh(fake_image)
        # origin_output = fake_image
        # output = fake_image
        ...
        # else:
        #     fake_image = self.netG.forward(input_concat)
        return img_hole_hand, arm_label_map, warped_cloth, skin_color, origin_output, output, real_image, cloth


    def train_transfer_input(self, model_g, model_d, input, target, label_cloth_mask, num_steps = 100):
        origin_output = model_g.refine(input)
        origin_output = self.tanh(origin_output)
        origin_output = origin_output.detach()
        output = [torch.zeros_like(origin_output, dtype = torch.float)]
        optimizer = torch.optim.LBFGS([input.requires_grad_()])
        run = [0]
        criterionVGG = networks.VGGLoss(self.gpu_ids)
        criterionGAN = networks.GANLoss(use_lsgan=not self.opt.no_lsgan, tensor=self.Tensor)
        criterionL1 = nn.L1Loss()
        #criterionL1 =  torch.nn.functional.mse_loss()
        style_weight = 0.1
        content_weight = 10
        gan_loss_weight = 0.1
        while run[0] < num_steps:
            def closure():
                ...
                optimizer.zero_grad()
                output[0] = model_g.refine(input)
                output[0] = self.tanh(output[0])
                output_cloth = output[0] * label_cloth_mask
                pred_fake_D = model_d.forward(torch.cat((input, output[0]), dim=1))
                style_score = 0
                content_score = 0
                loss_g_gan = criterionGAN(pred_fake_D, True) * gan_loss_weight

                style_loss = criterionVGG(output_cloth, target.detach())
                content_loss = criterionL1(output[0], origin_output.detach())
                style_score += style_loss
                content_score += content_loss
                #content_score += loss_g_gan
                #print("before style score = ", style_score)
                style_score *= style_weight
                #print("after style score = ", style_score)
                ...
                if run[0] % 10 == 0:
                    ...
                    print("Style loss : {:4f}   Content loss : {:4f}  gan loss : {:4f}  loss : {:4f}".format(style_loss.item(), content_loss.item(), loss_g_gan.item(), loss.item()))
                return style_score + content_score
            optimizer.step(closure)
        return origin_output, output[0]

    def  train_transfer_network(self, model_g, model_d, input, target, label_cloth_mask, num_steps = 100):
