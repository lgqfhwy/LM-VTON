...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, agnostic, cloth, cloth_mask, image, feat, infer=False):
        # Encode Inputs
        agnostic = agnostic.cuda()
        cloth = cloth.cuda()
        cloth_mask = cloth_mask.cuda()
        real_image = image.cuda()
        input_label = agnostic

        input_concat = torch.cat((agnostic, cloth), dim = 1)
    ...
    def inference(self, agnostic, cloth, cloth_mask, image=None):
        # Encode Inputs        
        agnostic = agnostic.cuda()
        cloth = cloth.cuda()
        cloth_mask = cloth_mask.cuda()
