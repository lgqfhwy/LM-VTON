...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.add_fake_mask = opt.add_fake_mask

        if self.add_fake_mask:
    ...
    def forward(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth, infer=False):
        ...
        img_hole_hand = real_image * all_label_person_shape * (1 - label_cloth_mask) * occlude * (1 - fake_cl_dis)
        #print("all_label_person_shape size = ", all_label_person_shape.size())
        ...
        if self.add_noise:
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color, self.gen_noise(label_cloth_mask.shape)], 1)
        else:
            G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color], 1)
        
        if self.add_fake_mask:
            G_in = torch.cat([G_in, fake_label_cloth_mask], 1)
        #G_in = torch.cat([img_hole_hand, dis_label, warped_cloth, skin_color], 1)
        ...
        loss_G_other = loss_l1 * self.l1_weight + loss_vgg * self.vgg_weight
    ...
    def inference(self, label_parse, warped_cloth, arm_label_map, fake_label_cloth_mask, remove_arm_parse, label_cloth_mask, real_image, pose, all_label_person_shape, cloth):
        ...
        occlude = (1 - bigger_arm1_occ * (arm2_mask + arm1_mask + fake_label_cloth_mask)) * (1 - bigger_arm2_occ * (arm2_mask + arm1_mask + fake_label_cloth_mask))
        img_hole_hand = real_image * (1 - label_cloth_mask) * occlude * (1 - fake_cl_dis) * all_label_person_shape
