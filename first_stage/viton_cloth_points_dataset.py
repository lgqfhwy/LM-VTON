...
class CPDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.root = "/data/lgq/database/viton_resize/"
        self.datamode = opt.datamode # train or test or self-defined
        self.stage = opt.stage # GMM or TOM
        #self.data_list = opt.data_list
        self.data_list = opt.datamode + "_pairs.txt"
        self.densepose_path = "densepose_viton_" + self.datamode + "_shape" 
        ...
        if self.is_pair:
            with open(osp.join(opt.dataroot, self.data_list), 'r') as f:
                for line in f.readlines():
                    ...
                    cur_c_name = im_name[:-5] + "1.jpg"
                    c_names.append(cur_c_name)
        else:
            with open(osp.join(opt.dataroot, self.data_list), 'r') as f:
                for line in f.readlines():
                    im_name, c_name = line.strip().split()
        ...
        train_cloth_path = osp.join(self.root, "cloth_points/viton_train_cloth.pkl")
        assert os.path.exists(train_cloth_path)
        person_cloth_path = osp.join(self.root, "cloth_points/viton_train_person.pkl")
    ...
    def __getitem__(self, index):
        ...
        densepose_shape = Image.open(osp.join(self.data_path, self.densepose_path, parse_name))
        ...
        # for i in range(len(c_point_plane)):
        #     #print("point i = ", c_point_plane[i])
        #     cv2.circle(origin_c, (int(c_point_plane[i][0]), int(c_point_plane[i][1])), 2, (0, 0, 255), 4)
        #     cv2.circle(orgin_im, (int(p_point_plane[i][0]), int(p_point_plane[i][1])), 2, (0, 0, 255), 4)
        # debug_c_path = os.path.join("./debug", c_name)
        # debug_p_path = os.path.join("./debug", im_name)
        # cv2.imwrite(debug_c_path, origin_c)
        # cv2.imwrite(debug_p_path, orgin_im)
