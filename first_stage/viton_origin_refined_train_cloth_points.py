...
import os
import time
from viton_cloth_points_dataset import CPDataset, CPDataLoader
...
def train_refined_gmm(opt, train_loader, model, board):
    ...
    if opt.use_mse_loss:
        warped_criterionL1 = nn.MSELoss()
        result_criterionL1 = nn.MSELoss()
    ...
    for step in range(opt.keep_step + opt.decay_step):
        ...
        target_shape = inputs['target_shape'].cuda()
        ...
        visuals = [ [im_h, densepose_shape, im_pose], 
                   [c, warped_cloth, im_c], 
                   [warped_grid, (warped_cloth+im)*0.5, im],
                   [m_composite, (c_result+im)*0.5, c_result]]
        ...
        # print("loss_c_result = ", loss_c_result)
        if opt.add_render_loss:
            loss = loss_warped_cloth + loss_weight * loss_point + loss_c_result + loss_mask + loss_vgg + loss_render + loss_gram
        else:
            loss = loss_warped_cloth + loss_weight * loss_point + loss_c_result + loss_mask + loss_vgg + loss_gram
        if opt.add_mask_constrain:
            #print("here")
            loss += loss_mask_constrain
        if opt.add_warped_mask_loss:
            loss += warped_mask_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
