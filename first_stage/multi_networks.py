...
class GramLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.layids is None:
            self.layids = list(range(len(x_vgg)))
        for i in self.layids:
            loss += self.weights[i] * self.criterion(self.gram(x_vgg[i]), self.gram(y_vgg[i].detach()))
        return loss
...
class GMM(nn.Module):
    ...
    def forward(self, inputA, inputB):
        ...
        return grid, theta
class RefinedGMM(nn.Module):
    """ Geometric Matching Module
    """
    def __init__(self, opt):
        super(RefinedGMM, self).__init__()
        input_nc = opt.gmm_feature_A_input_nc + 3
        output_nc = 4
        self.extractionA = FeatureExtraction(opt.gmm_feature_A_input_nc, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d) 
        self.extractionB = FeatureExtraction(3, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d)
        self.l2norm = FeatureL2Norm()
        self.correlation = FeatureCorrelation()
        self.regression = FeatureRegression(input_nc=192, output_dim=2*opt.grid_size**2, use_cuda=True)
    ...
    def forward(self, inputA, inputB):
        ...
        conv1 = self.conv1(torch.cat([inputA.detach(), warped_cloth.detach()], 1))
...
class OneRefinedGMM(nn.Module):
    ...
    def forward(self, inputA, inputB):
        ...
        if self.opt.add_origin_cloth:
            conv1 = self.conv1(torch.cat([origin_inputA, origin_inputB, warped_cloth], 1))
        else:
...
class TwoStageRefinedGMM(nn.Module):
    """ Geometric Matching Module
    """
    def __init__(self, opt):
        super(TwoStageRefinedGMM, self).__init__()
        input_nc = opt.gmm_feature_A_input_nc + 3
        output_nc = 4
        self.opt = opt
        if self.opt.add_origin_cloth:
